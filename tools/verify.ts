...
import { to_mnemonic } from '#src/index.ts';
...
async function main ([ file = './index.html' ]: Iterable<string>) {

    const base = new URL(
        is_http(file) ? file : import.meta.resolve(join(cwd(), file))
    );
    ...
    const mnemonic = await to_mnemonic(entropy);
...
function * collect (
    ...
    if (puzzle) {
        yield Promise.try(decodeBase58, puzzle);
    }
